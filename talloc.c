...
#define TALLOC_MAGIC_NON_RANDOM                                                        \
        (~TALLOC_FLAG_MASK & (TALLOC_MAGIC_BASE + (TALLOC_BUILD_VERSION_MAJOR << 24) + \
...
static void
talloc_log_stderr(const char *message)
{
...
*/
static inline void *
__talloc_with_prefix(const void *context, size_t size, size_t prefix_len, struct talloc_chunk **tc_ret)
{
...
_talloc_reference_loc(const void *context, const void *ptr, const char *location)
{
        ...
        tc = talloc_chunk_from_ptr(ptr);
        handle = _talloc_named_const(context, sizeof(struct talloc_reference_handle), TALLOC_MAGIC_REFERENCE);
...
static inline void _tc_free_children_internal(struct talloc_chunk *tc, void *ptr, const char *location);
                ...
                if (chunk_count == 1) {
                        ...
                        if (space_left >= space_needed) {
...
#if defined(DEVELOPER) && defined(VALGRIND_MAKE_MEM_UNDEFINED)
                                {
                                         ...
                                         */
                                        size_t undef_len =
                                            MIN((((char *)tc) - ((char *)new_ptr)), old_used);
...
*/
_PUBLIC_ void
talloc_report_depth_cb(const void *ptr,
                       int         depth,
                       int         max_depth,
...
talloc_report_depth_FILE_helper(const void *ptr, int depth, int max_depth, int is_ref, void *_f)
{
        ...
        if (depth == 0) {
                fprintf(f, "%stalloc report on '%s' (total %6llu bytes in %3llu blocks)\n",
                        (max_depth < 0 ? "full " : ""), name, (unsigned long long)talloc_total_size(ptr),
                        (unsigned long long)talloc_total_blocks(ptr));
                return;
        }

        fprintf(f, "%*s%-30s contains %6llu bytes in %3llu blocks (ref %d) %p\n", depth * 4, "",
                name, (unsigned long long)talloc_total_size(ptr),
...
*/
static inline size_t
_talloc_total_limit_size(const void *ptr, struct talloc_memlimit *old_limit, struct talloc_memlimit *new_limit)
{
